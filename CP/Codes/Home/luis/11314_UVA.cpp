...
int main()
{
	...
	while( tc-- )
	{
		...
		cin >> P.x >> P.y >> Q.x >> Q.y;
		if( P.x > Q.x )swap( P , Q );
		Point SP = Point( -P.x, P.y ) , SQ = Point( Q.x , -Q.y );
		Point OP = lineIntersection( SP , SQ , Point(0,0), Point(0,1) );
		Point OQ = lineIntersection( SP , SQ , Point(0,0), Point(1,0) );
		double ans = dist( P , Q ) + dist( Q , OQ ) + dist( OQ , OP ) + dist( OP , P ) ;
		printf( "%.3lf\n" , ans );
	}
}
